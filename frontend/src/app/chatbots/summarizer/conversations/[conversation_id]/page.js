...
const ConversationPage = ({ params }) => {
  ...
  const [conversationTitle, setConversationTitle] = useState('Summary Conversation');
  ...
  useEffect(() => {
    const fetchConversation = async () => {
      ...
      try {
        ...
        setConversationTitle(response.data.title || `Conversation ${conversation_id.slice(-4)}`);
      } catch (err) {
  ...
  // useEffect(() => {
  //   // Automatically send the summary as the first message
  //   const initialMessage = "Here is the summary of the article you provided.";
  //   setMessages((prev) => [
  //     ...prev,
    ...
    <div className="flex flex-col h-full bg-gray-100">      
      ...
      <div ref={messageListRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-white">
        {messages.map((msg, index) => (
          <div key={index} className={`flex ${msg.sender === user_id ? "justify-end" : "justify-start"}`}>
            <div className={`px-4 py-2 rounded-lg ${msg.sender === user_id ? "bg-blue-500 text-white" : "bg-gray-300 text-black"}`}>
              {msg.text}
            </div>
          </div>
        ))}
